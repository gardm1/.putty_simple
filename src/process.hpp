...
namespace prcs {
	...
	const wchar_t* string_to_pconstwchar_t(std::string s, ULONG nChars) {
#ifdef UTF_8_ENCODED_STRINGS
		wchar_t* wideString = new wchar_t[nChars];

		int result = MultiByteToWideChar(CP_UTF8, 0, s.c_str(), -1, wideString, nChars);
	...
	const wchar_t* pchar_to_pconstwchar_t(const char* c, ULONG nChars) {
#ifdef UTF_8_ENCODED_STRINGS
		wchar_t* wideString = new wchar_t[nChars];

		int result = MultiByteToWideChar(CP_UTF8, 0, c, -1, wideString, nChars);
	...
	void EXECUTECOMMAND(const char* sargv) {
		...
		const wchar_t* pArgs = pchar_to_pconstwchar_t(sargv, process.nChars);

		wcscpy_s(process.args, process.nChars, pArgs);
	...
	int __cdecl PING_INET_ADDR(char* argv) {
